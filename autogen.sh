...
[ -f configure.in ] || {
  echo "autogen.sh: run this command only at the top of a KAKASI source tree."
  exit 1
}
...
(autoconf --version) < /dev/null > /dev/null 2>&1 || {
  echo
  echo "You must have autoconf installed to compile KAKASI."
...
(automake --version) < /dev/null > /dev/null 2>&1 || {
  echo
  echo "You must have automake installed to compile KAKASI."
