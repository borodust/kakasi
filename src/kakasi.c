/*
 * $Id: kakasi.c,v 1.45 2015-04-10 08:18:28 knok Exp $
 * Copyright (C) 1992
...
int romaji_legacy_mode = 0; /* false */
int use_bs4812 = 0;	    /* false */
  ...
  int retval = 0;
	...
	case 'b':
	  ...
	  default:
	    ...
	    fprintf(stderr, "      -t: use the oldest romaji table\n");
	    fprintf(stderr, "      -z: use old romaji table\n");
	    fprintf(stderr, "      -b: use BS4812 romaji table (a hepburn variant)\n");
#ifdef WAKATIGAKI
    ...
    /* env check */
    if (getenv("KAKASI_LEGACY_ROMAJI") != NULL)
	romaji_legacy_mode = 1; /* true */
